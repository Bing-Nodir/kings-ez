...
export default function LoginPage() {
  ...
  async function handleSubmit(e) {
    ...
    setLoading(false);
    if (res?.error) return toast.error("Email yoki parol noto'g'ri");
  ...
  return (
    <div className="min-h-screen flex items-center justify-center px-4 py-16"
      style={{ background: "radial-gradient(ellipse at 30% 50%, rgba(0,188,212,.08), transparent 60%), radial-gradient(ellipse at 70% 20%, rgba(245,166,35,.06), transparent 60%)" }}>
      ...
      <div className="w-full max-w-md">
        ...
        <div className="glass p-8 rounded-2xl">
          ...
          <button onClick={handleGoogle} disabled={loading}
            className="w-full flex items-center justify-center gap-3 border border-white/10 rounded-xl py-3 text-sm font-medium text-white/80 hover:bg-white/5 hover:border-white/20 transition-all mb-5 disabled:opacity-50">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <button type="submit" disabled={loading} className="btn-primary w-full justify-center py-3 disabled:opacity-50">
              {loading ? "Kirish..." : <><LogIn className="w-4 h-4" /> Kirish</>}
            </button>
          </form>
